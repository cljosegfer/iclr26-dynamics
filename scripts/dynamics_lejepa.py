...
class LeJEPA_Model(nn.Module):
    ...
    def forward(self, x_t, action):
        ...
        z_hat_next = self.predictor(torch.cat([z_t, a_emb], dim=1))
...
def train(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if val_auc > best_val_auc:
            ...
            save_checkpoint(model, probe, optimizer, scheduler, scaler, epoch, best_val_loss, best_val_auc, "checkpoints/lejepa_probe.pth")
            print(">>> Saved probe Model")
        
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            save_checkpoint(model, probe, optimizer, scheduler, scaler, epoch, best_val_loss, best_val_auc, "checkpoints/lejepa_loss.pth")
            print(">>> Saved loss Model")
        
        # 1. Save Latest (For resuming)
        save_checkpoint(model, probe, optimizer, scheduler, scaler, epoch, best_val_loss, best_val_auc, "checkpoints/lejepa_latest.pth")
