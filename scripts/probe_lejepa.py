...
def get_features(model, loader, desc="Extracting"):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=desc):
            ...
            # feats = model.encode_target(x)
            # Manually replicate the forward pass for h
...
def main(args):
    ...
    print(f"Detected Feature Dimension: {feat_dim}") # Should be 256
    ...
    print(f"Best Val AUROC: {best_val_auc:.4f} at Epoch {best_epoch+1} and checkpoint {args.checkpoint_path}")
