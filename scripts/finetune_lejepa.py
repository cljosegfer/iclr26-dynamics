...
def load_pretrained_weights(model, checkpoint_path):
    """
    Adapts LeJEPA checkpoint to Standard ResNet classifier.
    """
    print(f"Loading LeJEPA weights from: {checkpoint_path}")
    checkpoint = torch.load(checkpoint_path, map_location='cpu')
    state_dict = checkpoint['model_state_dict']
    
    new_state_dict = {}
    for k, v in state_dict.items():
        # LeJEPA saves keys as: 'backbone.0.weight', 'backbone.1.0.weight', etc.
        # The Classifier (xresnet1d50) expects: '0.weight', '1.0.weight'
        if k.startswith('backbone.'):
            new_key = k.replace('backbone.', '')
            new_state_dict[new_key] = v
            
    # Load into model
    # strict=False is REQUIRED because:
    # 1. We are missing the classification head weights (final layers)
    # 2. We are ignoring 'projector', 'predictor', 'action_mlp' from the checkpoint
    msg = model.load_state_dict(new_state_dict, strict=False)
    
    print(f"   > Loaded. Missing keys (Head): {len(msg.missing_keys)}")
    # Verify we actually loaded something (unexpected keys should be 0 ideally, 
    # but since we filtered dict, they won't show up in msg.unexpected_keys)
    print(f"   > Transferred {len(new_state_dict)} layers.")
...
def train(args):
    ...
    # 3. Optimization
    # Low LR for Finetuning is crucial to preserve LeJEPA features
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_path', type=str, required=True, help="Path to lejepa_latest.pth")
